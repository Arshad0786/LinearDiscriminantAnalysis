...
for i in range(100):  # transform attributes from pandas dataframe to list
    if (i < 50):
...
B = u1-u2*np.transpose(u1-u2)

# calculate scatter
S1 = np.array([[0, 0], [0, 0]])
S2 = np.array([[0, 0], [0, 0]])
for i in range(50):
    S1 = S1 + (np.array(Iris_versicolor_list[i]) - u1) * \
        np.transpose((np.array(Iris_versicolor_list[i]) - u1))
    S2 = S2 + (np.array(Iris_setosa_list[i]) - u1) * \
...
for i in range(50):  # find all points
